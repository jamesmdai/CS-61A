    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if insect.impostor:
                    if insect.place.ant.is_container:
                        insect.place.ant.contained_ant = None
                    else:
                        self.ant = None
                    insect.place = None    
                return
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while position is not beehive:
            if position.bees and differential in range(self.min_range, self.max_range+1):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        dmg = amount
        beelst = self.place.bees[:]
        Ant.reduce_armor(self, amount)
        if self.armor<=0:
            dmg+=self.damage
        for bee in beelst:
            bee.reduce_armor(dmg)
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        if self.contained_ant and self.contained_ant.name == 'Harvester':
            self.contained_ant.action(colony)
        bee_lst = self.place.bees[:]
        for bee in bee_lst:
            bee.reduce_armor(self.damage)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while position:
                ant = position.ant
                if ant and not isinstance(ant, QueenAnt):
                    if isinstance(ant, BodyguardAnt) and ant.contained_ant and ant.contained_ant not in self.buffed_ants:
