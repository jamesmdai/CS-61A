...
def make_let_frame(bindings, env):
    ...
    params, vals= Pair(bindings.first.first, nil), Pair(scheme_eval(bindings.first.rest.first, env), nil)
    bindings = bindings.rest
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        params, vals = Pair(bindings.first.first, params), Pair(scheme_eval(bindings.first.rest.first, env), vals)
        bindings = bindings.rest

    check_formals(params)
    new_frame = env.make_child_frame(params, vals)
    return new_frame
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    fn = expressions.first.first
    params = expressions.first.rest
    body = expressions.rest
    env.define(fn, MacroProcedure(params, body, env))
    return fn
    # END Problem 20
